...
{
    public Task<IReadOnlyList<LeadListItem>> ListAsync(long? empresaId,long? puntoVentaId,string? status,int page,int pageSize,CancellationToken ct)=>repo.ListAsync(empresaId,puntoVentaId,status,page,pageSize,ct);
