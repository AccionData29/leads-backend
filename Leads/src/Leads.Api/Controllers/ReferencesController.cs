...
public sealed class ReferencesController(ReferenceQueries q) : ControllerBase
{
    ...
    {
        return Ok(await q.AdvisorsAsync(empresaId, puntoVentaId, activo, ct));
    }
    ...
    public async Task<IActionResult> Motorcycles(
        ...
        CancellationToken ct)
    {
        return Ok(await q.MotorcyclesAsync(marca, segmento, ct));
    }

    [HttpGet("pipeline-runs")]
    public async Task<IActionResult> PipelineRuns(
        [FromQuery] int page = 1,
        [FromQuery] int pageSize = 50,
        CancellationToken ct = default)
    {
        return Ok(await q.RunsAsync(page, pageSize, ct));
    }
}
